// 暴击与条件相关的通用工具（从 DiceCalculator 抽离，改为以 calc 实例为首参）

export function getRawDiceDistribution(calc, node) {
  if (!node) return {};
  if (node.type === 'dice' && node.isCriticalDice) return calc.calculateBasicDice(node.count, node.sides);
  if (node.type === 'dice_ref' && node.isCriticalDice) return calc.calculateBasicDice(node.count, node.sides);
  if (node.type === 'reroll' && node.dice && node.dice.isCriticalDice) return calc.calculateBasicDice(node.dice.count, node.dice.sides);
  if (node.type === 'exploding' && node.diceNode && node.diceNode.isCriticalDice) return calc.calculateBasicDice(node.diceNode.count, node.diceNode.sides);
  if (node.type === 'exploding_sum' && node.diceNode && node.diceNode.isCriticalDice) return calc.calculateBasicDice(node.diceNode.count, node.diceNode.sides);
  ...
  if (node.type === 'keep' && node.expressions) {
    for (const expr of node.expressions) {
      if (expr.type === 'dice' && expr.isCriticalDice) return calc.calculateBasicDice(1, expr.sides);
...
export function calculateConstantContribution(calc, node) {
  ...
  if (node.type === 'keep' && node.expressions) {
    let total = 0;
    for (const expr of node.expressions) {
      if (!calc.containsCriticalDice(expr)) total += calculateConstantContribution(calc, expr);
    }
    return total;
  }
  if (node.type === 'group' && node.expression) return calculateConstantContribution(calc, node.expression);
  return 0;
}
...
export function canProduceResult(calc, node, rawDiceValue, finalResult) {
  ...
  if (node.type === 'exploding' && node.diceNode && node.diceNode.isCriticalDice) return finalResult >= 0;
  if (node.type === 'exploding_sum' && node.diceNode && node.diceNode.isCriticalDice) return finalResult >= rawDiceValue;
  if (node.type === 'binary_op') {
    const left = node.left; const right = node.right;
    ...
    for (const expr of node.expressions) {
      if (calc.containsCriticalDice(expr)) {
        if (expr.type === 'dice' && expr.isCriticalDice) {
          ...
          if (keepType === 'highest') {
            if (finalResult >= rawDiceValue) return finalResult <= diceSides;
          } else if (keepType === 'lowest') {
            if (finalResult <= rawDiceValue) return finalResult >= 1;
          }
        } else {
...
export function findCriticalDiceInNode(calc, node) {
  if (!node) return null;
  if (node.type === 'dice' && node.isCriticalDice) return { sides: node.sides || 20, count: node.count || 1 };
  if (node.type === 'dice_ref' && node.isCriticalDice) return { sides: node.sides || 20, count: node.count || 1 };
  if (node.type === 'binary_op') {
    const leftResult = findCriticalDiceInNode(calc, node.left); if (leftResult) return leftResult;
    const rightResult = findCriticalDiceInNode(calc, node.right); if (rightResult) return rightResult;
  }
  if (node.type === 'keep' && node.expressions) {
    for (const expr of node.expressions) {
      const result = findCriticalDiceInNode(calc, expr);
...
export function getDiceInfoFromCondition(calc, conditionNode) {
  const findCriticalDice = (node) => {
    ...
    if (node.left) { const left = findCriticalDice(node.left); if (left) return left; }
    if (node.right) { const right = findCriticalDice(node.right); if (right) return right; }
    return null;
  };
...
export function evaluateConditionWithCritical(calc, conditionNode, actualCriticalProbability) {
  ...
  if (Object.keys(rightDistribution).length === 1 && Object.keys(rightDistribution)[0] !== undefined) {
      ...
      if (success) {
        ...
        if (leftHasCriticalDice && Object.keys(rawDiceDistribution).length > 0) {
          for (const [rawDiceValue] of Object.entries(rawDiceDistribution)) {
            ...
            if (rawVal >= criticalThreshold && canProduceResult(calc, conditionNode.left, rawVal, leftVal)) { isCritical = true; break; }
          }
        } else if (rightHasCriticalDice && Object.keys(rawDiceDistribution).length > 0) {
          for (const [rawDiceValue] of Object.entries(rawDiceDistribution)) {
            const rawVal = parseInt(rawDiceValue);
            if (rawVal >= criticalThreshold && canProduceResult(calc, conditionNode.right, rawVal, rightValue)) { isCritical = true; break; }
          }
        }
    ...
    for (const [rightValue, rightCount] of Object.entries(rightDistribution)) {
      ...
      if (success) {
        ...
        if (leftHasCriticalDice && Object.keys(rawDiceDistribution).length > 0) {
          for (const [rawDiceValue] of Object.entries(rawDiceDistribution)) {
            const rawVal = parseInt(rawDiceValue);
            if (rawVal >= criticalThreshold && canProduceResult(calc, conditionNode.left, rawVal, leftValue)) { isCritical = true; break; }
          }
        } else if (rightHasCriticalDice && Object.keys(rawDiceDistribution).length > 0) {
          for (const [rawDiceValue] of Object.entries(rawDiceDistribution)) {
            const rawVal = parseInt(rawDiceValue);
            if (rawVal >= criticalThreshold && canProduceResult(calc, conditionNode.right, rawVal, rightVal)) { isCritical = true; break; }
          }
        }
    ...
    for (const [leftValStr, leftCount] of Object.entries(leftDistribution)) {
      for (const [rightValStr, rightCount] of Object.entries(rightDistribution)) {
        const combinedCount = leftCount * rightCount;
        const leftValue = parseFloat(leftValStr);
        const rightValue = parseFloat(rightValStr);
        let success = false;
        switch (conditionNode.operator) {
